...
execute in daegl:mines positioned 5953 -6 -4149 run function daegl:util/create_fake_explosion 

scoreboard players set num_attacks daegl.boss_attack 3
schedule function daegl:dungeon/boss/start_next_attack 30s

execute in daegl:mines run spawnpoint @a[tag=in_daegl] 5854 -20 -4146

kill @e[tag=daegl.start_level_tutorial]
kill @e[tag=brewer_merchant]

execute as @e[tag=daegl.gold_spawner] at @s run setblock ~ ~ ~ air replace
