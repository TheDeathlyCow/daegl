# asat any

schedule clear daegl:dungeon/boss/fire_attack/warn
schedule clear daegl:dungeon/boss/fire_attack/start
schedule clear daegl:dungeon/boss/fire_attack/attack

schedule clear daegl:dungeon/boss/explosion_attack/warn
schedule clear daegl:dungeon/boss/explosion_attack/start
schedule clear daegl:dungeon/boss/explosion_attack/attack
schedule clear daegl:dungeon/boss/explosion_attack/next_explosion

schedule clear daegl:dungeon/boss/poison_attack/warn
schedule clear daegl:dungeon/boss/poison_attack/start
schedule clear daegl:dungeon/boss/poison_attack/attack

schedule clear daegl:dungeon/boss/ender_attack/warn
schedule clear daegl:dungeon/boss/ender_attack/start
schedule clear daegl:dungeon/boss/ender_attack/attack