...
tellraw @s [{"text": "Click "},{"text": "here","underlined": true, "color": "green", "clickEvent": {"action": "suggest_command","value": "/data modify entity @e[distance=..1,type=marker,tag=daegl_reset_marker,sort=nearest,limit=1] data.structure set value \"daegl:reset/\""}}, " to enter this marker's structure name!"]